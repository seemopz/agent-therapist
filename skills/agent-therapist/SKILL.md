...
Ask **What?** (Optimize (Recommended if files exist, else Build new) / Build new / Undo last change) and
**Where?** (Both (Recommended inside a git repo, else Global) / Global / Project) in one call (question tool, clickable) – except when "What?" is already known
